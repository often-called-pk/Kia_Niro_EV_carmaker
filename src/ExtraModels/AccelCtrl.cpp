...
DesrAccelFunc_ACC(double dt)
{
    ...
    if (s->relvTarget.NearPnt.dv_p < 0) {
        ACC_ECU->Time2Collision = s->relvTarget.NearPnt.ds_p / -s->relvTarget.NearPnt.dv_p;
    } else {
    ...
    if (s->Targ_Dtct) {
       ...
       or if target stand still DSMIN: 20[m] distance */
        ACC_ECU->DesrDist = M_MAX(((Vehicle.v + s->relvTarget.NearPnt.dv_p) * ACC_ECU->DesrTGap), ACC_ECU->dsmin);
...
AccelCtrl_New(struct tInfos *Inf, void *pCfg, char const *kindkey, char const *IdKey)
{
    ...
    if (isSensorRequired) {
        s = iGetStrOpt(Inf, strcat(strcpy(buf, key), ".RefObjectSensorName"), "Vhcl.RadarL");
