...
MainThread_Init(void)
{
    ...
    SysStartTimer(SimCore.DeltaT);
#endif
#if (defined(WIN32) || defined(LINUX64)) && !defined(CM_HIL)
    if (RT_ACTIVE) {
        SysStartTimer(SimCore.DeltaT);
    }
#endif
...
MainThread_BeginCycle(unsigned long long CycleNo64)
{
    ...
    if (LogPoll(TimeGlobal - SimCore.Start.TimeGlob) != 1 && SimCore.State > SCState_Start
        && SimCore.State < SCState_End) {
        SimCore_State_Set(SCState_End);
        SimCore_State_Switch();
    }
