/*
 * cosimulation.c
 * - based on https://github.com/modelica/Reference-FMUs/blob/main/src/cosimulation.c
 */
    ...
    if (loggingOn) {
        for (size_t i = 0; i < nCategories; i++) {
            if (categories[i] == NULL) {
                ...
                comp->logErrors = true;
            } else if (strcmp(categories[i], "logDebug") == 0) {
                comp->logDebug = true;
            } else {
...
void logDebug(ModelInstance *comp, const char *message, ...) {

    if (!comp || !comp->logDebug) return;

    va_list args;
    va_start(args, message);
    logMessage(comp, OK, "logDebug", message, args);
    va_end(args);
}
