...
# Add a Characteristic Value to the Test Run
TestMgr additem Characteristic "Brake Distance Calculation" \
    ...
    -param {
	{RTexpr {(first () ? Qu::BrakeDist=0); DM.Long.StepIdx>0 ? BrakeDist=Delta2Ev(Car.Road.sRoad, change(DM.Brake), Car.v <=0.01)}}
    }

# Add a Criterion for evalualtion of the Test Run
TestMgr additem Criterion "Brake Distance" \
    -desc "Test Run evaluation is carried out on the basis of Brake Distance. Test Run is considered good if Brake Distance < 14m." \
    -good {[get BrakeDist] <  27.0} \
    -warn {[get BrakeDist] >= 27.0 && [get BrakeDist] < 32.0} \
    -bad  {[get BrakeDist] >= 32.0}

# Add a Diagram to the Test Run
